...
export const synchronizations = [
  ...
  // OntoGrade: Trigger evaluation when user clicks "OntoGrade" button
  {
    ...
    do: () => {
      ...
      if (activeDiagram && activeDiagram.content) {
        ...
        uiConcept.actions.showNotification({
          message: 'No active diagram to evaluate. Please select a diagram first.',
          type: 'error',
          duration: 5000
        });
      }
  ...
  // OntoGrade: Handle lifting failures
  {
    ...
    do: ({ diagramId, error }) => {
      ...
      uiConcept.actions.showNotification({
        message: `OntoGrade Error: ${error.userMessage || error.message}`,
        type: 'error',
        duration: 7000
      });
    },
  ...
  // OntoGrade: Handle large graph warnings
  {
    when: 'largeGraphWarning',
    from: mermaidLifter,
    do: ({ diagramId, nodeCount }) => {
      console.warn(`[Sync] Large diagram detected: ${nodeCount} nodes`);
      uiConcept.actions.showNotification({
        message: `Large diagram detected (${nodeCount} nodes). Evaluation may take longer.`,
        type: 'info',
        duration: 5000
      });
    },
